...
if (window.screenshotExtensionInjected) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    // 滚动截图相关消息处理
    else if (message.type === 'SCROLLSHOT_SELECT_REGION') {
      handleScrollShotSelectRegion().then(sendResponse);
      return true;
    } else if (message.type === 'SCROLLSHOT_GET_SELECTED_REGION') {
      sendResponse({ ok: scrollShotState.selectedRegion !== null, region: scrollShotState.selectedRegion });
    } else if (message.type === 'SCROLLSHOT_EXPAND_REGION_CONTENT_WIDTH') {
      handleExpandRegionContentWidth().then(sendResponse);
      return true;
    } else if (message.type === 'SCROLLSHOT_GET_METRICS') {
      sendResponse(getScrollMetrics());
    } else if (message.type === 'SCROLLSHOT_GET_SCROLL') {
      sendResponse(getScrollInfo(message.requireRegion));
    } else if (message.type === 'SCROLLSHOT_MARK_MANUAL_START') {
      scrollShotState.isActive = true;
      scrollShotState.hideFixed = message.hideFixed;
  ...
  // 滚动截图：选择区域
  async function handleScrollShotSelectRegion() {
    return new Promise((resolve) => {
      ...
      const onMouseUp = (e) => {
        ...
        if (rect.width > 50 && rect.height > 50) {
          scrollShotState.selectedRegion = {
  ...
  // 滚动截图：扩展区域到内容宽度
  function handleExpandRegionContentWidth() {
    if (!scrollShotState.selectedRegion) {
    ...
    scrollShotState.selectedRegion.x = 0;
    scrollShotState.selectedRegion.width = contentWidth;

    return Promise.resolve({ ok: true, region: scrollShotState.selectedRegion });
  }
  ...
  // 获取当前滚动信息
  function getScrollInfo(requireRegion) {
    if (requireRegion && !scrollShotState.selectedRegion) {
      return { ok: false, error: 'No region selected' };
    }
  ...
  // 移除滚动截图遮罩层
  function removeScrollShotOverlay() {
    const existing = document.querySelectorAll('[data-scrollshot-overlay]');
